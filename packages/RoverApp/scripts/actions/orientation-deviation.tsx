...
export const OrientationDeviation = ({ onFinish }: PageArg) => {
  ...
  const [deviationPitch, set_deviationPitch] = useState(0);
  const [deviationRoll, set_deviationRoll] = useState(0);
  const [deviationHeading, set_deviationHeading] = useState(0);
  ...
  const onConfirmConfig = () => {
    ...
    config.current.deviationPitch = deviationPitch;
    config.current.deviationRoll = deviationRoll;
    config.current.deviationHeading = deviationHeading;
  ...
  useEffect(() => {
    if (currentStep === 'calibration') {
      computeInterval = setInterval(function () {
        let headDeviation = -parseFloat(((maxHeading + minHeading) / 2).toFixed(1));
        if (headDeviation < -180) {
          headDeviation = 360 + headDeviation;
        }

        set_deviationHeading(headDeviation);
        set_deviationPitch(-parseFloat(((maxPitch + minPitch) / 2).toFixed(1)));
        set_deviationRoll(-parseFloat(((maxRoll + minRoll) / 2).toFixed(1)));
      }, 5000);
  ...
  return (
    <Box alignItems="center" width="100%" flexDirection="column">
      {currentStep === 'calibration' ? (
        <Box alignItems="center" width="100%" flexDirection="column">
          <Text>Don't move the rover until the script end.</Text>
          <Br />
          <Task
        ...
        >
          <Text>
            Heading deviation : <Text color={'blue'}>{deviationHeading}</Text>
          </Text>
          <Text>
            Pitch deviation : <Text color={'blue'}>{deviationPitch}</Text>
          </Text>
          <Text>
            Roll deviation : <Text color={'blue'}>{deviationRoll}</Text>
          </Text>
        </Confirm>
      )}
    </Box>
